...
func (f FuzzerSpec) QUICFuzzerInterface() http_fuzzer.Fuzzer {
	switch f.Fuzzer() {
	case 1:
	...
	for w := range workQueue {
		...
		for _, fuzzerObject := range work.Fuzzers {
			...
			for _, requestWord := range fuzzerObject.RequestWords {
				...
				startTime = time.Now()
				uncensoredRequest, uncensoredResponse, uncensoredErr := fuzzerObject.Spec.QUICFuzzerInterface().Fuzz(work.IP, work.Domain, http_fuzzer.RequestWord{
				...
				censoredRequest, censoredResponse, censoredErr := fuzzerObject.Spec.QUICFuzzerInterface().Fuzz(work.IP, work.Domain, http_fuzzer.RequestWord{
