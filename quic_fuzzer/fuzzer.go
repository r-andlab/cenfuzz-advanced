...
func CreateTLSConfig(requestWord RequestWord) *utls.Config {
	...
	if len(requestWord.Certificate) > 0 {
		...
		if err != nil {
			log.Println("[quic_fuzzer.CreateTLSConfig] Error parsing certificate")
		}
		if c.Subject.CommonName == "" {
			clientCertPEM, ClientCertKey, err := util.GenerateCertificate(requestWord.Servername)
			if err != nil {
				log.Println("[quic_fuzzer.CreateTLSConfig] Could not generate client certificate")
				log.Println(err)
				return &utls.Config{
	...
	if len(serverNameParts) > 1 {
		...
		if serverNameParts[1] == "omit" {
			...
			if err != nil {
				log.Println("[quic_fuzzer.CreateTLSConfig] Error converting string into integer (repeat)")
				log.Println(err)
