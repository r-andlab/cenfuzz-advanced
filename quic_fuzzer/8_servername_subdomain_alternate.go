...
func (s *ServernameSubdomainsAlternate) Init(all bool) []*RequestWord {
	var requestWords []*RequestWord
	var requestWord *RequestWord
	retries := 0
	if !all {
		for i := 0; i < config.NumberOfProbesPerTest; i++ {
			ServerNameAlternate := util.GenerateSubdomainsAlternatives()
			...
			if containsRequestWord(requestWords, requestWord) {
				...
				retries += 1
				if retries >= 10 {
					log.Println("[ServernameSubdomainsAlternate.Init] Could not find a new random value after 10 retries. Breaking.")
					break
				}
			} else {
		...
		servernameAllAlternatives := util.GenerateAllSubdomainsAlternatives()
